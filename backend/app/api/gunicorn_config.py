...
from app.core.utils import setup_logger, get_env_value
...
errorlog = os.getenv("ERROR_LOG", log_file_path)  # Default write to zerag.log
accesslog = os.getenv("ACCESS_LOG", log_file_path)  # Default write to zerag.log
...
logconfig_dict = {
    ...
    "filters": {
        "path_filter": {
            "()": "app.core.utils.ZeragPathFilter",
        },
    },
    "loggers": {
        "zerag": {
...
def on_starting(server):
    ...
    # Log the location of the ZeRAG log file
    print(f"ZeRAG log file: {log_file_path}\n")
...
def post_fork(server, worker):
    ...
    setup_logger("zerag", log_level, add_filter=True, log_file_path=log_file_path)

    # Set up zerag submodule loggers
    for name in logging.root.manager.loggerDict:
        if name.startswith("zerag."):
