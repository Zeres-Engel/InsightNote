...
import asyncio
from app.core import ZeRAG, QueryParam
from app.core.utils import TiktokenTokenizer
from app.api.utils_api import get_combined_auth_dependency
from fastapi import Depends


# query mode according to query prefix (bypass is not ZeRAG quer mode)
...
class OllamaAPI:
    def __init__(self, rag: ZeRAG, top_k: int = 60, api_key: Optional[str] = None):
        self.rag = rag
    ...
    def setup_routes(self):
        ...
        async def get_tags():
            ...
            return OllamaTagResponse(
                models=[
                    {
                        "name": self.ollama_server_infos.ZERAG_MODEL,
                        "model": self.ollama_server_infos.ZERAG_MODEL,
                        "modified_at": self.ollama_server_infos.ZERAG_CREATED_AT,
                        "size": self.ollama_server_infos.ZERAG_SIZE,
                        "digest": self.ollama_server_infos.ZERAG_DIGEST,
                        "details": {
                            "parent_model": "",
                            "format": "gguf",
                            "family": self.ollama_server_infos.ZERAG_NAME,
                            "families": [self.ollama_server_infos.ZERAG_NAME],
                            "parameter_size": "13B",
                ...
                models=[
                    {
                        ...
                        "size_vram": self.ollama_server_infos.ZERAG_SIZE,
                    }
                ]
            )
        ...
        async def generate(raw_request: Request):
            ...
            For compatibility purpose, the request is not processed by ZeRAG,
                ...
                if request.stream:
                    ...
                    async def stream_generator():
                        ...
                        if isinstance(response, str):
                            ...
                            data = {
                                "model": self.ollama_server_infos.ZERAG_MODEL,
                                "created_at": self.ollama_server_infos.ZERAG_CREATED_AT,
                                "response": response,
                                "done": False,
                            }
        ...
        async def chat(raw_request: Request):
            ...
            Routes user queries through ZeRAG by selecting query mode based on query prefix.
