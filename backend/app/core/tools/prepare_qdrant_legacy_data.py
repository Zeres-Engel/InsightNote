#!/usr/bin/env python3
"""
Qdrant Legacy Data Preparation Tool for ZeRAG
...
Usage:
    python -m zerag.tools.prepare_qdrant_legacy_data
    # or
    python zerag/tools/prepare_qdrant_legacy_data.py

    # Specify custom workspace
    python -m zerag.tools.prepare_qdrant_legacy_data --workspace space1

    # Process specific collection types only
    python -m zerag.tools.prepare_qdrant_legacy_data --types chunks,entities

    # Dry run (preview only, no actual changes)
    python -m zerag.tools.prepare_qdrant_legacy_data --dry-run
"""
...
COLLECTION_NAMESPACES = {
    "chunks": {
        "new": "zerag_vdb_chunks",
        "suffix": "chunks",
    },
    "entities": {
        "new": "zerag_vdb_entities",
        "suffix": "entities",
    },
    "relationships": {
        "new": "zerag_vdb_relationships",
...
class QdrantLegacyDataPreparationTool:
    ...
    def print_header(self):
        """Print tool header"""
        print("\n" + "=" * 60)
        print("Qdrant Legacy Data Preparation Tool - ZeRAG")
        print("=" * 60)
        if self.dry_run:
