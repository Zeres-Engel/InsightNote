...
def download_tiktoken_cache(cache_dir: str = None, models: list = None):
    ...
    # Common models used by ZeRAG and OpenAI
    if models is None:
        models = [
            "gpt-4o-mini",  # Default model for ZeRAG
            "gpt-4o",  # GPT-4 Omni
            "gpt-4",  # GPT-4
            ...
            "cl100k_base",  # Default encoding for ZeRAG
        ]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="zerag-download-cache",
        description="Download cache files for ZeRAG offline deployment",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Download to default location (~/.tiktoken_cache)
  zerag-download-cache

  # Download to specific directory
  zerag-download-cache --cache-dir ./offline_cache/tiktoken

  # Download specific models only
  zerag-download-cache --models gpt-4o-mini gpt-4

For more information, visit: https://github.com/HKUDS/ZeRAG
        """,
    )
    ...
    parser.add_argument(
        "--version", action="version", version="%(prog)s (ZeRAG cache downloader)"
    )

    args = parser.parse_args()

    print("=" * 70)
    print("ZeRAG Offline Cache Downloader")
    print("=" * 70)
