#!/usr/bin/env python3
"""
Diagnostic tool to check ZeRAG initialization status.

This tool helps developers verify that their ZeRAG instance is properly
...
Usage:
    ...
    await check_zerag_setup(rag, verbose=True)

    # Run demo from command line:
    python -m zerag.tools.check_initialization --demo
"""
...
from app.core.base import StoragesStatus


async def check_zerag_setup(rag_instance: ZeRAG, verbose: bool = False) -> bool:
    """
    Check if a ZeRAG instance is properly initialized.

    Args:
        rag_instance: The ZeRAG instance to check
    ...
    print("🔍 Checking ZeRAG initialization status...\n")

    # Check storage initialization status
    if not hasattr(rag_instance, "_storages_status"):
        issues.append("ZeRAG instance missing _storages_status attribute")
    ...
    if issues:
        ...
        print(
            "\n📚 Documentation: https://github.com/HKUDS/ZeRAG#important-initialization-requirements"
        )
        ...
        print("✅ ZeRAG is properly initialized and ready to use!")
...
async def demo():
    ...
    from app.core.llm.openai import openai_embed, gpt_4o_mini_complete

    print("=" * 50)
    print("ZeRAG Initialization Diagnostic Tool")
    print("=" * 50)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Check ZeRAG initialization status")
    parser.add_argument(
    ...
    if args.demo:
        ...
        print("Or import this module and use check_zerag_setup() with your instance")
