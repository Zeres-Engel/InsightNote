#!/usr/bin/env python3
"""
LLM Query Cache Cleanup Tool for ZeRAG
...
Usage:
    python -m zerag.tools.clean_llm_query_cache
    # or
    python zerag/tools/clean_llm_query_cache.py
...
from app.core.kg import STORAGE_ENV_REQUIREMENTS
from app.core.kg.shared_storage import set_all_update_flags
...
class CleanupTool:
    ...
    def get_storage_class(self, storage_name: str):
        ...
        if storage_name == "JsonKVStorage":
            ...
            from app.core.kg.redis_impl import RedisKVStorage

            return RedisKVStorage
        elif storage_name == "PGKVStorage":
            from app.core.kg.postgres_impl import PGKVStorage

            return PGKVStorage
        elif storage_name == "MongoKVStorage":
            from app.core.kg.mongo_impl import MongoKVStorage
    ...
    async def count_query_caches_pg(self, storage) -> Dict[str, Dict[str, int]]:
        ...
        Returns:
            Dictionary with counts for each mode and cache_type
        """
        from app.core.kg.postgres_impl import namespace_to_table_name
    ...
    def print_header(self):
        """Print tool header"""
        print("\n" + "=" * 60)
        print("LLM Query Cache Cleanup Tool - ZeRAG")
        print("=" * 60)
    ...
    async def setup_storage(self) -> tuple:
        ...
        if storage_name == "JsonKVStorage":
            ...
            print("  • ZeRAG Server is completely shut down")
            print("  • No other programs are accessing the storage files")
            print("\n" + "=" * 60)

            confirm = (
                input("\nHas ZeRAG Server been shut down? (yes/no): ")
                .strip()
                .lower()
            )
            if confirm != "yes":
                print(
                    "\n✓ Operation cancelled - Please shut down ZeRAG Server first"
                )
                return None, None, None
    ...
    async def run(self):
        ...
        try:
            ...
            from app.core.kg.shared_storage import initialize_share_data
            ...
            try:
                from app.core.kg.shared_storage import finalize_share_data
