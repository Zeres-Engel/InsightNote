...
from app.core.types import KnowledgeGraph, KnowledgeGraphNode, KnowledgeGraphEdge
...
class PostgreSQLDB:
    ...
    async def _migrate_llm_cache_schema(self):
        ...
        try:
            ...
            WHERE table_name = 'zerag_llm_cache'
            ...
            if "chunk_id" not in existing_column_names:
                logger.info("Adding chunk_id column to ZERAG_LLM_CACHE table")
                add_chunk_id_sql = """
                ALTER TABLE ZERAG_LLM_CACHE
                ADD COLUMN chunk_id VARCHAR(255) NULL
                """
                await self.execute(add_chunk_id_sql)
                logger.info(
                    "Successfully added chunk_id column to ZERAG_LLM_CACHE table"
                )
            else:
                logger.info(
                    "chunk_id column already exists in ZERAG_LLM_CACHE table"
                )

            # Add missing cache_type column
            if "cache_type" not in existing_column_names:
                logger.info("Adding cache_type column to ZERAG_LLM_CACHE table")
                add_cache_type_sql = """
                ALTER TABLE ZERAG_LLM_CACHE
                ...
                logger.info(
                    "Successfully added cache_type column to ZERAG_LLM_CACHE table"
                )
                ...
                logger.info(
                    "cache_type column already exists in ZERAG_LLM_CACHE table"
                )

            # Add missing queryparam column
            if "queryparam" not in existing_column_names:
                logger.info("Adding queryparam column to ZERAG_LLM_CACHE table")
                add_queryparam_sql = """
                ALTER TABLE ZERAG_LLM_CACHE
                ...
                logger.info(
                    "Successfully added queryparam column to ZERAG_LLM_CACHE table"
                )
            else:
                logger.info(
                    "queryparam column already exists in ZERAG_LLM_CACHE table"
                )

            # Remove deprecated mode field if it exists
            if "mode" in existing_column_names:
                logger.info(
                    "Removing deprecated mode column from ZERAG_LLM_CACHE table"
                )

                # First, drop the primary key constraint that includes mode
                drop_pk_sql = """
                ALTER TABLE ZERAG_LLM_CACHE
                DROP CONSTRAINT IF EXISTS ZERAG_LLM_CACHE_PK
                """
                ...
                logger.info(
                    "Successfully removed mode column from ZERAG_LLM_CACHE table"
                )

                # Create new primary key constraint without mode
                add_pk_sql = """
                ALTER TABLE ZERAG_LLM_CACHE
                ADD CONSTRAINT ZERAG_LLM_CACHE_PK PRIMARY KEY (workspace, id)
                """
                await self.execute(add_pk_sql)
                logger.info("Created new primary key constraint (workspace, id)")
            else:
                logger.info("mode column does not exist in ZERAG_LLM_CACHE table")
    ...
    async def _migrate_timestamp_columns(self):
        ...
        tables_to_migrate = {
            "ZERAG_VDB_ENTITY": ["create_time", "update_time"],
            "ZERAG_VDB_RELATION": ["create_time", "update_time"],
            "ZERAG_DOC_CHUNKS": ["create_time", "update_time"],
            "ZERAG_DOC_STATUS": ["created_at", "updated_at"],
        }
    ...
    async def _migrate_doc_chunks_to_vdb_chunks(self):
        """
        Migrate data from ZERAG_DOC_CHUNKS to ZERAG_VDB_CHUNKS if specific conditions are met.
        This migration is intended for users who are upgrading and have an older table structure
        where ZERAG_DOC_CHUNKS contained a `content_vector` column.

        """
        try:
            # 0. Check if both tables exist before proceeding
            vdb_chunks_exists = await self.check_table_exists("ZERAG_VDB_CHUNKS")
            doc_chunks_exists = await self.check_table_exists("ZERAG_DOC_CHUNKS")

            if not vdb_chunks_exists:
                logger.debug(
                    "Skipping migration: ZERAG_VDB_CHUNKS table does not exist"
                )
                return

            if not doc_chunks_exists:
                logger.debug(
                    "Skipping migration: ZERAG_DOC_CHUNKS table does not exist"
                )
                return

            # 1. Check if the new table ZERAG_VDB_CHUNKS is empty
            vdb_chunks_count_sql = "SELECT COUNT(1) as count FROM ZERAG_VDB_CHUNKS"
            ...
            if vdb_chunks_count_result and vdb_chunks_count_result["count"] > 0:
                logger.info(
                    "Skipping migration: ZERAG_VDB_CHUNKS already contains data."
                )
                return
            ...
            WHERE table_name = 'zerag_doc_chunks' AND column_name = 'content_vector'
            """
            column_exists = await self.query(check_column_sql)
            if not column_exists:
                logger.info(
                    "Skipping migration: `content_vector` not found in ZERAG_DOC_CHUNKS"
                )
                return

            # 3. Check if the old table ZERAG_DOC_CHUNKS has data
            doc_chunks_count_sql = "SELECT COUNT(1) as count FROM ZERAG_DOC_CHUNKS"
            ...
            if not doc_chunks_count_result or doc_chunks_count_result["count"] == 0:
                logger.info("Skipping migration: ZERAG_DOC_CHUNKS is empty.")
                return

            # 4. Perform the migration
            logger.info(
                "Starting data migration from ZERAG_DOC_CHUNKS to ZERAG_VDB_CHUNKS..."
            )
            migration_sql = """
            INSERT INTO ZERAG_VDB_CHUNKS (
            ...
            SELECT
                id, workspace, full_doc_id, chunk_order_index, tokens, content,
                content_vector, file_path, create_time, update_time
            FROM ZERAG_DOC_CHUNKS
            ON CONFLICT (workspace, id) DO NOTHING;
            """
            await self.execute(migration_sql)
            logger.info("Data migration to ZERAG_VDB_CHUNKS completed successfully.")

        except Exception as e:
            logger.error(f"Failed during data migration to ZERAG_VDB_CHUNKS: {e}")
    ...
    async def _migrate_llm_cache_to_flattened_keys(self):
        ...
        try:
            ...
            check_sql = """
            SELECT COUNT(*) as count FROM ZERAG_LLM_CACHE
            WHERE id NOT LIKE '%:%'
            """
            result = await self.query(check_sql)
            ...
            WITH new_ids AS (
                ...
                FROM ZERAG_LLM_CACHE
                WHERE id NOT LIKE '%:%'
            )
            SELECT COUNT(*) as conflicts
            FROM new_ids n1
            JOIN ZERAG_LLM_CACHE existing
            ...
            verify_sql = """
            SELECT COUNT(*) as remaining_old FROM ZERAG_LLM_CACHE
            WHERE id NOT LIKE '%:%'
            """
    ...
    async def _migrate_doc_status_add_chunks_list(self):
        """Add chunks_list column to ZERAG_DOC_STATUS table if it doesn't exist"""
        try:
            # Check if chunks_list column exists
            check_column_sql = """
            SELECT column_name
            FROM information_schema.columns
            WHERE table_name = 'zerag_doc_status'
            AND column_name = 'chunks_list'
            """

            column_info = await self.query(check_column_sql)
            if not column_info:
                logger.info("Adding chunks_list column to ZERAG_DOC_STATUS table")
                add_column_sql = """
                ALTER TABLE ZERAG_DOC_STATUS
                ADD COLUMN chunks_list JSONB NULL DEFAULT '[]'::jsonb
                """
                await self.execute(add_column_sql)
                logger.info(
                    "Successfully added chunks_list column to ZERAG_DOC_STATUS table"
                )
            else:
                logger.info(
                    "chunks_list column already exists in ZERAG_DOC_STATUS table"
                )
        except Exception as e:
            logger.warning(
                f"Failed to add chunks_list column to ZERAG_DOC_STATUS: {e}"
            )

    async def _migrate_text_chunks_add_llm_cache_list(self):
        """Add llm_cache_list column to ZERAG_DOC_CHUNKS table if it doesn't exist"""
        try:
            # Check if llm_cache_list column exists
            check_column_sql = """
            SELECT column_name
            FROM information_schema.columns
            WHERE table_name = 'zerag_doc_chunks'
            AND column_name = 'llm_cache_list'
            """

            column_info = await self.query(check_column_sql)
            if not column_info:
                logger.info("Adding llm_cache_list column to ZERAG_DOC_CHUNKS table")
                add_column_sql = """
                ALTER TABLE ZERAG_DOC_CHUNKS
                ADD COLUMN llm_cache_list JSONB NULL DEFAULT '[]'::jsonb
                """
                await self.execute(add_column_sql)
                logger.info(
                    "Successfully added llm_cache_list column to ZERAG_DOC_CHUNKS table"
                )
            else:
                logger.info(
                    "llm_cache_list column already exists in ZERAG_DOC_CHUNKS table"
                )
        except Exception as e:
            logger.warning(
                f"Failed to add llm_cache_list column to ZERAG_DOC_CHUNKS: {e}"
            )

    async def _migrate_doc_status_add_track_id(self):
        """Add track_id column to ZERAG_DOC_STATUS table if it doesn't exist and create index"""
        try:
            ...
            column_info = await self.query(check_column_sql)
            if not column_info:
                logger.info("Adding track_id column to ZERAG_DOC_STATUS table")
                add_column_sql = """
                ALTER TABLE ZERAG_DOC_STATUS
                ...
                await self.execute(add_column_sql)
                logger.info(
                    "Successfully added track_id column to ZERAG_DOC_STATUS table"
                )
            else:
                logger.info(
                    "track_id column already exists in ZERAG_DOC_STATUS table"
                )
            ...
            SELECT indexname
            FROM pg_indexes
            WHERE tablename = 'zerag_doc_status'
            AND indexname = 'idx_zerag_doc_status_track_id'
            """

            index_info = await self.query(check_index_sql)
            if not index_info:
                logger.info(
                    "Creating index on track_id column for ZERAG_DOC_STATUS table"
                )
                create_index_sql = """
                CREATE INDEX idx_zerag_doc_status_track_id ON ZERAG_DOC_STATUS (track_id)
                """
                await self.execute(create_index_sql)
                logger.info(
                    "Successfully created index on track_id column for ZERAG_DOC_STATUS table"
                )
            else:
                logger.info(
                    "Index on track_id column already exists for ZERAG_DOC_STATUS table"
                )

        except Exception as e:
            logger.warning(
                f"Failed to add track_id column or index to ZERAG_DOC_STATUS: {e}"
            )

    async def _migrate_doc_status_add_metadata_error_msg(self):
        """Add metadata and error_msg columns to ZERAG_DOC_STATUS table if they don't exist"""
        try:
            ...
            if not metadata_info:
                logger.info("Adding metadata column to ZERAG_DOC_STATUS table")
                add_metadata_sql = """
                ALTER TABLE ZERAG_DOC_STATUS
                ...
                logger.info(
                    "Successfully added metadata column to ZERAG_DOC_STATUS table"
                )
            else:
                logger.info(
                    "metadata column already exists in ZERAG_DOC_STATUS table"
                )
            ...
            if not error_msg_info:
                logger.info("Adding error_msg column to ZERAG_DOC_STATUS table")
                add_error_msg_sql = """
                ALTER TABLE ZERAG_DOC_STATUS
                ...
                logger.info(
                    "Successfully added error_msg column to ZERAG_DOC_STATUS table"
                )
            else:
                logger.info(
                    "error_msg column already exists in ZERAG_DOC_STATUS table"
                )

        except Exception as e:
            logger.warning(
                f"Failed to add metadata/error_msg columns to ZERAG_DOC_STATUS: {e}"
            )
    ...
    async def check_tables(self):
        ...
        # Create pagination optimization indexes for ZERAG_DOC_STATUS
        try:
            await self._create_pagination_indexes()
        except Exception as e:
            logger.error(f"PostgreSQL, Failed to create pagination indexes: {e}")

        # Migrate to ensure new tables ZERAG_FULL_ENTITIES and ZERAG_FULL_RELATIONS exist
        try:
    ...
    async def _migrate_create_full_entities_relations_tables(self):
        """Create ZERAG_FULL_ENTITIES and ZERAG_FULL_RELATIONS tables if they don't exist"""
        tables_to_check = [
            {
                "name": "ZERAG_FULL_ENTITIES",
                "ddl": TABLES["ZERAG_FULL_ENTITIES"]["ddl"],
                "description": "Full entities storage table",
            },
            {
                "name": "ZERAG_FULL_RELATIONS",
                "ddl": TABLES["ZERAG_FULL_RELATIONS"]["ddl"],
    ...
    async def _create_pagination_indexes(self):
        """Create indexes to optimize pagination queries for ZERAG_DOC_STATUS"""
        indexes = [
            {
                "name": "idx_zerag_doc_status_workspace_status_updated_at",
                "sql": "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_zerag_doc_status_workspace_status_updated_at ON ZERAG_DOC_STATUS (workspace, status, updated_at DESC)",
                "description": "Composite index for workspace + status + updated_at pagination",
            },
            {
                "name": "idx_zerag_doc_status_workspace_status_created_at",
                "sql": "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_zerag_doc_status_workspace_status_created_at ON ZERAG_DOC_STATUS (workspace, status, created_at DESC)",
                "description": "Composite index for workspace + status + created_at pagination",
            },
            {
                "name": "idx_zerag_doc_status_workspace_updated_at",
                "sql": "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_zerag_doc_status_workspace_updated_at ON ZERAG_DOC_STATUS (workspace, updated_at DESC)",
                "description": "Index for workspace + updated_at pagination (all statuses)",
            },
            {
                "name": "idx_zerag_doc_status_workspace_created_at",
                "sql": "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_zerag_doc_status_workspace_created_at ON ZERAG_DOC_STATUS (workspace, created_at DESC)",
                "description": "Index for workspace + created_at pagination (all statuses)",
            },
            {
                "name": "idx_zerag_doc_status_workspace_id",
                "sql": "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_zerag_doc_status_workspace_id ON ZERAG_DOC_STATUS (workspace, id)",
                "description": "Index for workspace + id sorting",
            },
            {
                "name": "idx_zerag_doc_status_workspace_file_path",
                "sql": "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_zerag_doc_status_workspace_file_path ON ZERAG_DOC_STATUS (workspace, file_path)",
...
class PGVectorStorage(BaseVectorStorage):
    ...
    async def query(
        ...
        params = {
            "workspace": self.workspace,
            "closer_than_threshold": 1 - (score_threshold if score_threshold is not None else 0.2),
            "top_k": top_k,
        }
...
class PGDocStatusStorage(DocStatusStorage):
    ...
    async def get_by_id(self, id: str) -> Union[dict[str, Any], None]:
        sql = "select * from ZERAG_DOC_STATUS where workspace=$1 and id=$2"
        params = {"workspace": self.workspace, "id": id}
    ...
    async def get_by_ids(self, ids: list[str]) -> list[dict[str, Any]]:
        ...
        sql = "SELECT * FROM ZERAG_DOC_STATUS WHERE workspace=$1 AND id = ANY($2)"
        params = {"workspace": self.workspace, "ids": ids}
    ...
    async def get_doc_by_file_path(self, file_path: str) -> Union[dict[str, Any], None]:
        ...
        """
        sql = "select * from ZERAG_DOC_STATUS where workspace=$1 and file_path=$2"
    ...
    async def get_docs_by_status(
        ...
        sql = "select * from ZERAG_DOC_STATUS where workspace=$1 and status=$2"
    ...
    async def get_docs_by_track_id(
        ...
        sql = "select * from ZERAG_DOC_STATUS where workspace=$1 and track_id=$2"
    ...
    async def get_docs_paginated(
        ...
        count_sql = f"SELECT COUNT(*) as total FROM ZERAG_DOC_STATUS {where_clause}"
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        sql = """insert into ZERAG_DOC_STATUS(workspace,id,content_summary,content_length,chunks_count,status,file_path,chunks_list,track_id,metadata,error_msg,created_at,updated_at)
...
class PGGraphStorage(BaseGraphStorage):
    ...
    async def upsert_edge(
        ...
        # Build Cypher query with dynamic dollar-quoting to handle content containing $$
        # This prevents syntax errors when LLM-extracted descriptions contain $ sequences
        # See: https://github.com/HKUDS/ZeRAG/issues/1438#issuecomment-2826000195
...
NAMESPACE_TABLE_MAP = {
    NameSpace.KV_STORE_FULL_DOCS: "ZERAG_DOC_FULL",
    NameSpace.KV_STORE_TEXT_CHUNKS: "ZERAG_DOC_CHUNKS",
    NameSpace.KV_STORE_FULL_ENTITIES: "ZERAG_FULL_ENTITIES",
    NameSpace.KV_STORE_FULL_RELATIONS: "ZERAG_FULL_RELATIONS",
    NameSpace.KV_STORE_ENTITY_CHUNKS: "ZERAG_ENTITY_CHUNKS",
    NameSpace.KV_STORE_RELATION_CHUNKS: "ZERAG_RELATION_CHUNKS",
    NameSpace.KV_STORE_LLM_RESPONSE_CACHE: "ZERAG_LLM_CACHE",
    NameSpace.VECTOR_STORE_CHUNKS: "ZERAG_VDB_CHUNKS",
    NameSpace.VECTOR_STORE_ENTITIES: "ZERAG_VDB_ENTITY",
    NameSpace.VECTOR_STORE_RELATIONSHIPS: "ZERAG_VDB_RELATION",
    NameSpace.DOC_STATUS: "ZERAG_DOC_STATUS",
}
...
TABLES = {
    "ZERAG_DOC_FULL": {
        "ddl": """CREATE TABLE ZERAG_DOC_FULL (
                    id VARCHAR(255),
	                ...
	                CONSTRAINT ZERAG_DOC_FULL_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "ZERAG_DOC_CHUNKS": {
        "ddl": """CREATE TABLE ZERAG_DOC_CHUNKS (
                    id VARCHAR(255),
                    workspace VARCHAR(255),
                    ...
                    create_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
                    update_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
	                CONSTRAINT ZERAG_DOC_CHUNKS_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "ZERAG_VDB_CHUNKS": {
        "ddl": """CREATE TABLE ZERAG_VDB_CHUNKS (
                    id VARCHAR(255),
                    workspace VARCHAR(255),
                    full_doc_id VARCHAR(256),
                    chunk_order_index INTEGER,
                    ...
                    create_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
                    update_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
	                CONSTRAINT ZERAG_VDB_CHUNKS_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "ZERAG_VDB_ENTITY": {
        "ddl": """CREATE TABLE ZERAG_VDB_ENTITY (
                    id VARCHAR(255),
                    workspace VARCHAR(255),
                    ...
                    file_path TEXT NULL,
	                CONSTRAINT ZERAG_VDB_ENTITY_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "ZERAG_VDB_RELATION": {
        "ddl": """CREATE TABLE ZERAG_VDB_RELATION (
                    id VARCHAR(255),
                    workspace VARCHAR(255),
                    ...
                    chunk_ids VARCHAR(255)[] NULL,
                    file_path TEXT NULL,
	                CONSTRAINT ZERAG_VDB_RELATION_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "ZERAG_LLM_CACHE": {
        "ddl": """CREATE TABLE ZERAG_LLM_CACHE (
	                ...
	                CONSTRAINT ZERAG_LLM_CACHE_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "ZERAG_DOC_STATUS": {
        "ddl": """CREATE TABLE ZERAG_DOC_STATUS (
	               workspace varchar(255) NOT NULL,
	               id varchar(255) NOT NULL,
	               ...
	               CONSTRAINT ZERAG_DOC_STATUS_PK PRIMARY KEY (workspace, id)
	              )"""
    },
    "ZERAG_FULL_ENTITIES": {
        "ddl": """CREATE TABLE ZERAG_FULL_ENTITIES (
                    id VARCHAR(255),
                    workspace VARCHAR(255),
                    entity_names JSONB,
                    count INTEGER,
                    create_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
                    update_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
                    CONSTRAINT ZERAG_FULL_ENTITIES_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "ZERAG_FULL_RELATIONS": {
        "ddl": """CREATE TABLE ZERAG_FULL_RELATIONS (
                    id VARCHAR(255),
                    workspace VARCHAR(255),
                    ...
                    create_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
                    update_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
                    CONSTRAINT ZERAG_FULL_RELATIONS_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "ZERAG_ENTITY_CHUNKS": {
        "ddl": """CREATE TABLE ZERAG_ENTITY_CHUNKS (
                    id VARCHAR(512),
                    workspace VARCHAR(255),
                    chunk_ids JSONB,
                    count INTEGER,
                    create_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
                    update_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
                    CONSTRAINT ZERAG_ENTITY_CHUNKS_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "ZERAG_RELATION_CHUNKS": {
        "ddl": """CREATE TABLE ZERAG_RELATION_CHUNKS (
                    id VARCHAR(512),
                    workspace VARCHAR(255),
                    chunk_ids JSONB,
                    count INTEGER,
                    create_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
                    update_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
                    CONSTRAINT ZERAG_RELATION_CHUNKS_PK PRIMARY KEY (workspace, id)
                    )"""
    },
}
...
SQL_TEMPLATES = {
    ...
    "get_by_id_full_docs": """SELECT id, COALESCE(content, '') as content,
                                ...
                                FROM ZERAG_DOC_FULL WHERE workspace=$1 AND id=$2
                            """,
    "get_by_id_text_chunks": """SELECT id, tokens, COALESCE(content, '') as content,
                                ...
                                FROM ZERAG_DOC_CHUNKS WHERE workspace=$1 AND id=$2
                            """,
    "get_by_id_llm_response_cache": """SELECT id, original_prompt, return_value, chunk_id, cache_type, queryparam,
                                EXTRACT(EPOCH FROM create_time)::BIGINT as create_time,
                                EXTRACT(EPOCH FROM update_time)::BIGINT as update_time
                                FROM ZERAG_LLM_CACHE WHERE workspace=$1 AND id=$2
                               """,
    "get_by_ids_full_docs": """SELECT id, COALESCE(content, '') as content,
                                 COALESCE(doc_name, '') as file_path
                                 FROM ZERAG_DOC_FULL WHERE workspace=$1 AND id = ANY($2)
                            """,
    "get_by_ids_text_chunks": """SELECT id, tokens, COALESCE(content, '') as content,
                                  ...
                                  EXTRACT(EPOCH FROM create_time)::BIGINT as create_time,
                                  EXTRACT(EPOCH FROM update_time)::BIGINT as update_time
                                   FROM ZERAG_DOC_CHUNKS WHERE workspace=$1 AND id = ANY($2)
                                """,
    "get_by_ids_llm_response_cache": """SELECT id, original_prompt, return_value, chunk_id, cache_type, queryparam,
                                 EXTRACT(EPOCH FROM create_time)::BIGINT as create_time,
                                 EXTRACT(EPOCH FROM update_time)::BIGINT as update_time
                                 FROM ZERAG_LLM_CACHE WHERE workspace=$1 AND id = ANY($2)
                                """,
    "get_by_id_full_entities": """SELECT id, entity_names, count,
                                EXTRACT(EPOCH FROM create_time)::BIGINT as create_time,
                                EXTRACT(EPOCH FROM update_time)::BIGINT as update_time
                                FROM ZERAG_FULL_ENTITIES WHERE workspace=$1 AND id=$2
                               """,
    "get_by_id_full_relations": """SELECT id, relation_pairs, count,
                                EXTRACT(EPOCH FROM create_time)::BIGINT as create_time,
                                EXTRACT(EPOCH FROM update_time)::BIGINT as update_time
                                FROM ZERAG_FULL_RELATIONS WHERE workspace=$1 AND id=$2
                               """,
    "get_by_ids_full_entities": """SELECT id, entity_names, count,
                                 EXTRACT(EPOCH FROM create_time)::BIGINT as create_time,
                                 EXTRACT(EPOCH FROM update_time)::BIGINT as update_time
                                 FROM ZERAG_FULL_ENTITIES WHERE workspace=$1 AND id = ANY($2)
                                """,
    "get_by_ids_full_relations": """SELECT id, relation_pairs, count,
                                 EXTRACT(EPOCH FROM create_time)::BIGINT as create_time,
                                 EXTRACT(EPOCH FROM update_time)::BIGINT as update_time
                                 FROM ZERAG_FULL_RELATIONS WHERE workspace=$1 AND id = ANY($2)
                                """,
    "get_by_id_entity_chunks": """SELECT id, chunk_ids, count,
                                EXTRACT(EPOCH FROM create_time)::BIGINT as create_time,
                                EXTRACT(EPOCH FROM update_time)::BIGINT as update_time
                                FROM ZERAG_ENTITY_CHUNKS WHERE workspace=$1 AND id=$2
                               """,
    "get_by_id_relation_chunks": """SELECT id, chunk_ids, count,
                                EXTRACT(EPOCH FROM create_time)::BIGINT as create_time,
                                EXTRACT(EPOCH FROM update_time)::BIGINT as update_time
                                FROM ZERAG_RELATION_CHUNKS WHERE workspace=$1 AND id=$2
                               """,
    "get_by_ids_entity_chunks": """SELECT id, chunk_ids, count,
                                 EXTRACT(EPOCH FROM create_time)::BIGINT as create_time,
                                 EXTRACT(EPOCH FROM update_time)::BIGINT as update_time
                                 FROM ZERAG_ENTITY_CHUNKS WHERE workspace=$1 AND id = ANY($2)
                                """,
    "get_by_ids_relation_chunks": """SELECT id, chunk_ids, count,
                                 EXTRACT(EPOCH FROM create_time)::BIGINT as create_time,
                                 EXTRACT(EPOCH FROM update_time)::BIGINT as update_time
                                 FROM ZERAG_RELATION_CHUNKS WHERE workspace=$1 AND id = ANY($2)
                                """,
    "filter_keys": "SELECT id FROM {table_name} WHERE workspace=$1 AND id IN ({ids})",
    "upsert_doc_full": """INSERT INTO ZERAG_DOC_FULL (id, content, doc_name, workspace)
                        VALUES ($1, $2, $3, $4)
                        ON CONFLICT (workspace,id) DO UPDATE
                           SET content = $2,
                               ...
                               update_time = CURRENT_TIMESTAMP
                       """,
    "upsert_llm_response_cache": """INSERT INTO ZERAG_LLM_CACHE(workspace,id,original_prompt,return_value,chunk_id,cache_type,queryparam)
                                      ...
                                      update_time = CURRENT_TIMESTAMP
                                     """,
    "upsert_text_chunk": """INSERT INTO ZERAG_DOC_CHUNKS (workspace, id, tokens,
                     ...
                     """,
    "upsert_full_entities": """INSERT INTO ZERAG_FULL_ENTITIES (workspace, id, entity_names, count,
                      create_time, update_time)
                      VALUES ($1, $2, $3, $4, $5, $6)
                      ON CONFLICT (workspace,id) DO UPDATE
                      ...
                      update_time = EXCLUDED.update_time
                     """,
    "upsert_full_relations": """INSERT INTO ZERAG_FULL_RELATIONS (workspace, id, relation_pairs, count,
                      create_time, update_time)
                      VALUES ($1, $2, $3, $4, $5, $6)
                      ON CONFLICT (workspace,id) DO UPDATE
                      ...
                      update_time = EXCLUDED.update_time
                     """,
    "upsert_entity_chunks": """INSERT INTO ZERAG_ENTITY_CHUNKS (workspace, id, chunk_ids, count,
                      create_time, update_time)
                      VALUES ($1, $2, $3, $4, $5, $6)
                      ON CONFLICT (workspace,id) DO UPDATE
                      ...
                      update_time = EXCLUDED.update_time
                     """,
    "upsert_relation_chunks": """INSERT INTO ZERAG_RELATION_CHUNKS (workspace, id, chunk_ids, count,
                      create_time, update_time)
                      VALUES ($1, $2, $3, $4, $5, $6)
                      ON CONFLICT (workspace,id) DO UPDATE
                      SET chunk_ids=EXCLUDED.chunk_ids,
