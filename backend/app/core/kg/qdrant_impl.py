...
def _find_legacy_collection(
    ...
    created by older versions of ZeRAG:

    1. zerag_vdb_{namespace} - if model_suffix is provided (HIGHEST PRIORITY)
    2. {workspace}_{namespace} or {namespace} - no matter if model_suffix is provided or not
    3. zerag_vdb_{namespace} - fall back value no matter if model_suffix is provided or not (LOWEST PRIORITY)
...
class QdrantVectorDBStorage(BaseVectorStorage):
    ...
    def __post_init__(self):
        ...
        # Simple naming: {workspace}_{namespace}
        self.final_namespace = f"{self.effective_workspace}_{self.namespace}"
        ...
        async with get_data_init_lock():
            ...
            try:
                ...
                if self._client is None:
                    # Suppress the "Api key is used with an insecure connection" warning from Qdrant client
                    # when using an API key over an HTTP connection instead of HTTPS.
    ...
    async def query(
        ...
        results = self._client.query_points(
            ...
            limit=top_k,
            with_payload=True,
            score_threshold=score_threshold if score_threshold is not None else 0.2,
