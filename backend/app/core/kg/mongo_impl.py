        ...
        async with cls._lock:
            if cls._instances["db"] is None:
                ...
                database_name = os.environ.get(
                    "MONGO_DATABASE",
                    config.get("mongodb", "database", fallback="ZeRAG"),
                )
...
class MongoVectorDBStorage(BaseVectorStorage):
    ...
    async def query(
        ...
        # Check for user-provided score_threshold or fallback to 0.2
        effective_threshold = score_threshold if score_threshold is not None else 0.2
        ...
        pipeline = [
            ...
            {"$match": {"score": {"$gte": effective_threshold}}},
            {"$project": {"vector": 0}},
        ]
