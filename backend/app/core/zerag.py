...
@dataclass
class ZeRAG:
    """ZeRAG: Simple and Fast Retrieval-Augmented Generation."""

    # Directory
    # ---

    working_dir: str = field(default="./data")
    ...
    vector_db_storage_cls: type[BaseVectorStorage] = field(default=None)  # type: ignore
    """Storage backend for vector embeddings. Set automatically from `vector_storage` in `__post_init__`."""
    ...
    # TODO: Deprecated (ZeRAG will never initialize storage automatically on creation，and finalize should be call before destroying)
    auto_manage_storages_states: bool = field(default=False)
    """If True, zerag will automatically calls initialize_storages and finalize_storages at the appropriate times."""
    ...
    # Graph readiness flag
    graph_ready: bool = field(default=False)
    ...
    def __post_init__(self):
        ...
        logger.debug(f"ZeRAG init with param:\n  {_print_config}\n")
        ...
        # while preventing side effects when the same EmbeddingFunc is reused across multiple ZeRAG instances
    ...
    async def initialize_storages(self):
        ...
        if self._storages_status == StoragesStatus.CREATED:
            ...
            try:
                ...
                logger.warning(
                    f"[{self.workspace}] Neo4j Graph initialization failed: {e}. "
                    "không xài được graph mode nha chỉ query được naive nhớ log báo đồ đầy đủ nha"
                )

            # Initialize remaining KG related KV storages if graph is ready
            # Note: even if graph fails, we might still want to initialize these for potential recovery
            # but usually they depend on the same backend.
            for storage_name, storage in [
    ...
    def _get_storage_class(self, storage_name: str) -> Callable[..., Any]:
        ...
        if storage_name == "JsonKVStorage":
            from app.core.kg.json_kv_impl import JsonKVStorage

            return JsonKVStorage
        elif storage_name == "NanoVectorDBStorage":
            from app.core.kg.nano_vector_db_impl import NanoVectorDBStorage

            return NanoVectorDBStorage
        elif storage_name == "NetworkXStorage":
            from app.core.kg.networkx_impl import NetworkXStorage

            return NetworkXStorage
        elif storage_name == "JsonDocStatusStorage":
            from app.core.kg.json_doc_status_impl import JsonDocStatusStorage
    ...
    async def ainsert(
        ...
        Args:
            ...
            track_id: tracking ID for monitoring processing status, if not provided, will be generated
            metadata: optional list of metadata for each document/chunk
    ...
    async def adrop(self) -> None:
        ...
        if hasattr(self, "full_docs") and hasattr(self.full_docs, "drop"):
            tasks.append(self.full_docs.drop())
        if hasattr(self, "doc_status") and hasattr(self.doc_status, "drop"):
        ...
        if hasattr(self, "chunk_entity_relation_graph") and hasattr(
            self.chunk_entity_relation_graph, "drop"
        ):
            tasks.append(self.chunk_entity_relation_graph.drop())
        if hasattr(self, "entity_chunks") and hasattr(self.entity_chunks, "drop"):
            tasks.append(self.entity_chunks.drop())
        if hasattr(self, "relation_chunks") and hasattr(self.relation_chunks, "drop"):
            tasks.append(self.relation_chunks.drop())

        if tasks:
            results = await asyncio.gather(*tasks, return_exceptions=True)
            for i, res in enumerate(results):
                if isinstance(res, Exception):
                    logger.error(f"Error dropping storage {i}: {res}")
        logger.info("All storages dropped.")
    ...
    async def ainsert_custom_chunks(
        ...
        try:
            ...
            entity_extraction_task = self._process_extract_entities(inserting_chunks)
            storage_tasks = [
                self.chunks_vdb.upsert(inserting_chunks),
                self.full_docs.upsert(new_docs),
                self.text_chunks.upsert(inserting_chunks),
            ]
            results = await asyncio.gather(entity_extraction_task, *storage_tasks)
            chunk_results = results[0]
    ...
    async def apipeline_enqueue_documents(
        ...
        # Compile a list of metadata corresponding to each document
        if metadata is not None:
            if not isinstance(metadata, list):
                metadata = [metadata]
            if len(metadata) != len(input):
                logger.warning(
                    f"Metadata length ({len(metadata)}) does not match input length ({len(input)}). Discarding metadata."
                )
                metadata_list = [{}] * len(input)
            else:
        ...
        if ignored_ids:
            ...
            for doc_id in ignored_ids:
                ...
                # Merge original metadata with duplicate tracking info
                original_metadata = new_docs.get(doc_id, {}).get("metadata", {})
                dup_metadata = {
                    **original_metadata,
                    "is_duplicate": True,
    ...
    async def apipeline_enqueue_file_reference(
        self,
        file_path: str,
        track_id: str | None = None,
        ...
        """
        Enqueue a file reference directly without extracting its content yet.
        ...
        if existing_doc:
            ...
            logger.warning(f"File {file_path} already enqueued with status {status}")
            return False, existing_doc.get("track_id", track_id)
        ...
        new_doc = {
            "status": DocStatus.PENDING,
            "content_summary": f"File reference: {os.path.basename(file_path)}",
        ...
        await self.doc_status.upsert({doc_id: new_doc})
        logger.info(f"Enqueued file reference: {file_path} (track_id: {track_id})")
    ...
    async def _validate_and_fix_document_consistency(
        ...
        for doc_id, status_doc in to_process_docs.items():
            ...
            if not content_data:
                # If there's no content_data, check if it has a valid file_path for dynamic extraction
                file_path = getattr(status_doc, "file_path", "unknown_source")
                if file_path and file_path != "unknown_source" and file_path != "":
                    # Valid file path exists, so it's consistent for dynamic processing
                    pass
                else:
        ...
        for doc_id, status_doc in to_process_docs.items():
            ...
            if content_data or (
                ...
                if hasattr(status_doc, "status") and status_doc.status in [
                    ...
                    # Do not reset duplicate documents
                    ...
                    if "graph_mode" not in metadata:
                        metadata = {**metadata, "graph_mode": False}
                    if "multi_modal" not in metadata:
                        metadata = {**metadata, "multi_modal": False}
                    # Update in-memory object so process_document reads correct metadata
                    ...
                    docs_to_reset[doc_id] = {
                        ...
                        # Clear any error messages but preserve processing metadata
                        "error_msg": "",
                        "metadata": metadata,
                    }
                ...
                async def process_document(
                    ...
                    async with semaphore:
                        ...
                        try:
                            ...
                            # Check if the document was intentionally marked as FAILED (e.g., duplicates)
                            ...
                            content_data = await self.full_docs.get_by_id(doc_id)
                            content = content_data["content"] if content_data else None
                            ...
                            if content is None:
                                ...
                                if (
                                    ...
                                    try:
                                        ...
                                        return  # Exit function, processing failed
                                else:
                                    # No content + no dynamic processing → error/orphan document, skip with FAILED status
                                    logger.warning(
                                        f"Document {doc_id} ({file_path}) has no content in full_docs and multi_modal={_is_multi_modal}. Marking as FAILED."
                                    )
                                    processing_end_time = int(time.time())
                                    await self.doc_status.upsert(
                                        {
                            ...
                            # Read original metadata (graph_mode, multi_modal, etc.) before any overwrite
                            doc_metadata = getattr(status_doc, "metadata", None)
                            if not isinstance(doc_metadata, dict):
                                doc_metadata = {}
                            # Legacy/corrupted docs: metadata was overwritten with only processing_start_time -> restore graph_mode/multi_modal
                            if "graph_mode" not in doc_metadata:
                                doc_metadata = {**doc_metadata, "graph_mode": False}
                            if "multi_modal" not in doc_metadata:
                                doc_metadata = {**doc_metadata, "multi_modal": False}
    ...
    async def aquery_data(
        ...
        if data_param.mode in ["local", "global", "hybrid", "mix"]:
            if not self.graph_ready:
                logger.warning(
                    f"[{self.workspace}] Graph storage is not ready. "
                    f"Falling back from '{data_param.mode}' to 'naive' mode."
                )
        ...
        try:
            ...
            if param.mode in ["local", "global", "hybrid", "mix"]:
                if not self.graph_ready:
                    logger.warning(
                        f"[{self.workspace}] Graph storage is not ready. "
                        f"Falling back from '{param.mode}' to 'naive' mode."
                    )
                    param.mode = "naive"
                else:
                    query_result = await kg_query(
    ...
    async def adelete_by_entity(self, entity_name: str) -> DeletionResult:
        ...
        Returns:
            DeletionResult: An object containing the outcome of the deletion process.
        """
        from app.core.utils_graph import adelete_by_entity
    ...
    async def adelete_by_relation(
        ...
        Returns:
            DeletionResult: An object containing the outcome of the deletion process.
        """
        from app.core.utils_graph import adelete_by_relation
    ...
    async def get_entity_info(
        ...
        from app.core.utils_graph import get_entity_info
    ...
    async def get_relation_info(
        ...
        from app.core.utils_graph import get_relation_info
    ...
    async def aedit_entity(
        ...
        """
        from app.core.utils_graph import aedit_entity
    ...
    async def aedit_relation(
        ...
        """
        from app.core.utils_graph import aedit_relation
    ...
    async def acreate_entity(
        ...
        """
        from app.core.utils_graph import acreate_entity
    ...
    async def acreate_relation(
        ...
        """
        from app.core.utils_graph import acreate_relation
    ...
    async def amerge_entities(
        ...
        """
        from app.core.utils_graph import amerge_entities
    ...
    async def aexport_data(
        ...
        """
        from app.core.utils import aexport_data as utils_aexport_data
