...
async def _get_vector_context(
    ...
    try:
        ...
        if not results:
            logger.info(
                f"Naive query: 0 chunks (chunk_top_k:{search_top_k} vector_score:{vector_score})"
            )
            return []
        ...
        logger.info(
            f"Naive query: {len(valid_chunks)} chunks (chunk_top_k:{search_top_k} vector_score:{vector_score})"
        )
        return valid_chunks
