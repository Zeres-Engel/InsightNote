#!/usr/bin/env python3
"""
RAGAS Evaluation Script for ZeRAG System
...
Usage:
    ...
    python zerag/evaluation/eval_rag_quality.py -d my_test.json

    # Specify custom RAG endpoint
    python zerag/evaluation/eval_rag_quality.py --ragendpoint http://my-server.com:9621
    python zerag/evaluation/eval_rag_quality.py -r http://my-server.com:9621
...
Results are saved to: zerag/evaluation/results/
...
class RAGEvaluator:
    ...
    def __init__(self, test_dataset_path: str = None, rag_api_url: str = None):
        ...
        Args:
            ...
            rag_api_url: Base URL of ZeRAG API (e.g., http://localhost:9621)
        ...
        if rag_api_url is None:
            rag_api_url = os.getenv("ZERAG_API_URL", "http://localhost:9621")
    ...
    def _display_configuration(self):
        ...
        logger.info("  • ZeRAG API:         %s", self.rag_api_url)
    ...
    async def generate_rag_response(
        ...
        """
        Generate RAG response by calling ZeRAG API.
        ...
        Raises:
            Exception: If ZeRAG API is unavailable.
        """
        try:
            payload = {
                "query": question,
            ...
            raise Exception(
                f"❌ Cannot connect to ZeRAG API at {self.rag_api_url}\n"
                f"   Make sure ZeRAG server is running:\n"
                f"   python -m app.api.zerag_server\n"
                f"   Error: {str(e)}"
            )
        except httpx.HTTPStatusError as e:
            raise Exception(
                f"ZeRAG API error {e.response.status_code}: {e.response.text}"
            )
            ...
            raise Exception(f"Error calling ZeRAG API: {type(e).__name__}: {str(e)}")
    ...
    async def evaluate_responses(self) -> List[Dict[str, Any]]:
        ...
        logger.info("%s", "=" * 70)
        logger.info("🚀 Starting RAGAS Evaluation of ZeRAG System")
...
async def main():
    ...
    Command-line arguments:
        ...
        --ragendpoint, -r: ZeRAG API endpoint URL (default: http://localhost:9621 or $ZERAG_API_URL)
    ...
    try:
        ...
        parser = argparse.ArgumentParser(
            description="RAGAS Evaluation Script for ZeRAG System",
...
Examples:
  ...
  python zerag/evaluation/eval_rag_quality.py -d my_test.json -r http://localhost:9621
        ...
        parser.add_argument(
            ...
            type=str,
            default=None,
            help="ZeRAG API endpoint URL (default: http://localhost:9621 or $ZERAG_API_URL environment variable)",
        )

        args = parser.parse_args()

        logger.info("%s", "=" * 70)
        logger.info("🔍 RAGAS Evaluation - Using Real ZeRAG API")
        logger.info("%s", "=" * 70)
