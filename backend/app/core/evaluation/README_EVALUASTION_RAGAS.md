...
### 📁 ZeRAG Evalua'tion Framework Directory Structure

```
zerag/evaluation/
├── eval_rag_quality.py      # Main evaluation script
├── sample_dataset.json        # 3 test questions about ZeRAG
├── sample_documents/          # Matching markdown files for testing
│   ├── 01_zerag_overview.md
│   ├── 02_rag_architecture.md
│   ├── 03_zerag_improvements.md
...
**Quick Test:** Index files from `sample_documents/` into ZeRAG, then run the evaluator to reproduce results (~89-100% RAGAS score per question).
...
```bash
cd /path/to/ZeRAG
python zerag/evaluation/eval_rag_quality.py
```

**Specify custom dataset:**
```bash
python zerag/evaluation/eval_rag_quality.py --dataset my_test.json
```

**Specify custom RAG endpoint:**
```bash
python zerag/evaluation/eval_rag_quality.py --ragendpoint http://my-server.com:9621
```

**Specify both (short form):**
```bash
python zerag/evaluation/eval_rag_quality.py -d my_test.json -r http://localhost:9621
```

**Get help:**
```bash
python zerag/evaluation/eval_rag_quality.py --help
```

### 3. View Results

Results are saved automatically in `zerag/evaluation/results/`:
...
| `--ragendpoint` | `-r` | `http://localhost:9621` or `$ZERAG_API_URL` | ZeRAG API endpoint URL |
...
```bash
python zerag/evaluation/eval_rag_quality.py --dataset path/to/my_dataset.json
```
...
```bash
python zerag/evaluation/eval_rag_quality.py -d my_dataset.json -r http://localhost:9621
```

**Absolute path to dataset:**
```bash
python zerag/evaluation/eval_rag_quality.py -d /path/to/custom_dataset.json
```
...
EVAL_QUERY_TOP_K=10      # OP_K query parameter of ZeRAG
...
`sample_dataset.json` contains 3 generic questions about ZeRAG. Replace with questions matching YOUR indexed documents.
...
### "ZeRAG query API errors during evaluation"

The evaluation uses your configured LLM (OpenAI by default). Ensure:
- API keys are set in `.env`
- Network connection is stable

### Evaluation requires running ZeRAG API

The evaluator queries a running ZeRAG API server at `http://localhost:9621`. Make sure:
1. ZeRAG API server is running (`python zerag/api/zerag_server.py`)
2. Documents are indexed in your ZeRAG instance
...
1. Start ZeRAG API server
2. Upload sample documents into ZeRAG  throught  WebUI
3. Run `python zerag/evaluation/eval_rag_quality.py`
...
```
INFO: ======================================================================
INFO: 🔍 RAGAS Evaluation - Using Real ZeRAG API
INFO: ======================================================================
...
INFO:   • ZeRAG API:         http://localhost:9621
INFO:   • Results Directory:    results
INFO: ======================================================================
INFO: 🚀 Starting RAGAS Evaluation of ZeRAG System
...
INFO: -------------------------------------------------------------------------------------------------------------------
INFO: 1    | How does ZeRAG solve the hallucination probl... | 1.0000 |  1.0000 | 1.0000 |  1.0000 | 1.0000 |      ✓
INFO: 2    | What are the three main components required in ... | 0.8500 |  0.5790 | 1.0000 |  1.0000 | 0.8573 |      ✓
INFO: 3    | How does ZeRAG's retrieval performance compa... | 0.8056 |  1.0000 | 1.0000 |  1.0000 | 0.9514 |      ✓
INFO: 4    | What vector databases does ZeRAG support and... | 0.8182 |  0.9807 | 1.0000 |  1.0000 | 0.9497 |      ✓
INFO: 5    | What are the four key metrics for evaluating RA... | 1.0000 |  0.7452 | 1.0000 |  1.0000 | 0.9363 |      ✓
INFO: 6    | What are the core benefits of ZeRAG and how ... | 0.9583 |  0.8829 | 1.0000 |  1.0000 | 0.9603 |      ✓
INFO: ===================================================================================================================
INFO:
INFO: ======================================================================
