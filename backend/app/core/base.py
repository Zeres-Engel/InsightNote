...
class OllamaServerInfos:
    def __init__(self, name=None, tag=None):
        self._zerag_name = name or os.getenv(
            "OLLAMA_EMULATING_MODEL_NAME", DEFAULT_OLLAMA_MODEL_NAME
        )
        self._zerag_tag = tag or os.getenv(
            "OLLAMA_EMULATING_MODEL_TAG", DEFAULT_OLLAMA_MODEL_TAG
        )
        self.ZERAG_SIZE = DEFAULT_OLLAMA_MODEL_SIZE
        self.ZERAG_CREATED_AT = DEFAULT_OLLAMA_CREATED_AT
        self.ZERAG_DIGEST = DEFAULT_OLLAMA_DIGEST

    @property
    def ZERAG_NAME(self):
        return self._zerag_name

    @ZERAG_NAME.setter
    def ZERAG_NAME(self, value):
        self._zerag_name = value

    @property
    def ZERAG_TAG(self):
        return self._zerag_tag

    @ZERAG_TAG.setter
    def ZERAG_TAG(self, value):
        self._zerag_tag = value

    @property
    def ZERAG_MODEL(self):
        return f"{self._zerag_name}:{self._zerag_tag}"
...
class TextChunkSchema(TypedDict):
    ...
    metadata: Optional[dict[str, Any]]  # For storing bbox, page_idx, etc.
...
@dataclass
class QueryParam:
    """Configuration parameters for query execution in ZeRAG."""
    ...
    """Cosine similarity score threshold for vector retrieval.
    Replaces the global cosine_better_than_threshold for this query.
    If not provided, the system default is used.
    """

    rerank_score: float | None = None
    """Minimum rerank score threshold for keeping reranked chunks.
    If not provided, the system default is used.
    """
...
class BaseVectorStorage(StorageNameSpace, ABC):
    ...
    def _generate_collection_suffix(self) -> str | None:
        ...
        if not hasattr(self.embedding_func, "model_name") or not self.embedding_func.model_name:
            return None

        # Clean model name: replace non-alphanumeric chars with underscore
        clean_name = re.sub(r'[^a-zA-Z0-9]', '_', self.embedding_func.model_name.lower())
        # Remove consecutive underscores
        clean_name = re.sub(r'_+', '_', clean_name).strip('_')

        return f"{clean_name}_{self.embedding_func.embedding_dim}d"

    @abstractmethod
    async def query(
        self, query: str, top_k: int, score_threshold: float = None, query_embedding=None
    ) -> list[dict]:
