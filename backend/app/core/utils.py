...
def _patch_ascii_colors_console_handler() -> None:
    ...
    if getattr(ConsoleHandler, "_zerag_patched", False):
        return
    ...
    ConsoleHandler._zerag_patched = True  # type: ignore[attr-defined]
...
if TYPE_CHECKING:
    from app.core.base import BaseKVStorage, BaseVectorStorage, QueryParam
...
class ZeragPathFilter(logging.Filter):
    """Filter for zerag logger to filter out frequent path access logs"""
...
def setup_logger(
    ...
    Args:
        ...
        add_filter: Whether to add ZeragPathFilter to the logger
        log_file_path: Path to the log file. If None and file logging is enabled, defaults to zerag.log in LOG_DIR or cwd
...
class EmbeddingFunc:
    ...
    Args:
        ...
        max_token_size: Enable embedding token limit checking for description summarization(Set embedding_token_limit in ZeRAG)
...
async def aexport_data(
    ...
    if file_format == "csv":
        ...
        with open(output_path, "w", encoding="utf-8") as mdfile:
            mdfile.write("# ZeRAG Data Export\n\n")
        ...
        with open(output_path, "w", encoding="utf-8") as txtfile:
            txtfile.write("ZERAG DATA EXPORT\n")
            txtfile.write("=" * 80 + "\n\n")
