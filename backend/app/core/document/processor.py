...
class ProcessorMixin:
    """ProcessorMixin class containing document processing functionality for MultiRAG"""
    ...
    async def _process_multimodal_content(
        ...
        # Check multimodal processing status - handle ZeRAG's early DocStatus.PROCESSED marking
        try:
            existing_doc_status = await self.zerag.doc_status.get_by_id(doc_id)
        ...
        try:
            # Ensure ZeRAG is initialized
            await self._ensure_zerag_initialized()
    ...
    async def _process_multimodal_content_individual(
        ...
        if multimodal_chunk_ids:
            try:
                ...
                current_doc_status = await self.zerag.doc_status.get_by_id(doc_id)
                ...
                if current_doc_status:
                    ...
                    await self.zerag.doc_status.upsert(
                        {
                            doc_id: {
                    ...
                    await self.zerag.doc_status.index_done_callback()
        ...
        if all_chunk_results:
            ...
            await merge_nodes_and_edges(
                ...
                knowledge_graph_inst=self.zerag.chunk_entity_relation_graph,
                entity_vdb=self.zerag.entities_vdb,
                relationships_vdb=self.zerag.relationships_vdb,
                global_config=self.zerag.__dict__,
                full_entities_storage=self.zerag.full_entities,
                full_relations_storage=self.zerag.full_relations,
                doc_id=doc_id,
                pipeline_status=pipeline_status,
                pipeline_status_lock=pipeline_status_lock,
                llm_response_cache=self.zerag.llm_response_cache,
                current_file_number=1,
                total_files=1,
                file_path=file_name,
            )

            await self.zerag._insert_done()
    ...
    async def _process_multimodal_content_batch_type_aware(
        ...
        # Use ZeRAG's concurrency control
        ...
        # Stage 2: Convert to ZeRAG chunks format
        zerag_chunks = self._convert_to_zerag_chunks_type_aware(
            multimodal_data_list, file_path, doc_id
        )

        # Stage 3: Store chunks to ZeRAG storage
        await self._store_chunks_to_zedata_type_aware(zerag_chunks)

        # Stage 3.5: Store multimodal main entities to entities_vdb and full_entities
        await self._store_multimodal_main_entities(
            multimodal_data_list, zerag_chunks, file_path, doc_id
        )

        # Track chunk IDs for doc_status update
        chunk_ids = list(zerag_chunks.keys())

        # Stage 4: Use ZeRAG's batch entity relation extraction
        chunk_results = await self._batch_extract_entities_zerag_style_type_aware(
            zerag_chunks
        )
        ...
        # Stage 6: Use ZeRAG's batch merge
        await self._batch_merge_zerag_style_type_aware(
    ...
    def _convert_to_zerag_chunks_type_aware(
        self, multimodal_data_list: List[Dict[str, Any]], file_path: str, doc_id: str
    ) -> Dict[str, Any]:
        """Convert multimodal data to ZeRAG standard chunks format"""
        ...
        for data in multimodal_data_list:
            ...
            # Calculate tokens
            tokens = len(self.zerag.tokenizer.encode(formatted_chunk_content))

            # Use full path or basename based on config
            file_ref = self._get_file_reference(file_path)

            page_idx = data["item_info"].get("page_idx", 0)
            bbox = original_item.get("bbox") or original_item.get("bb")

            # metadata for Qdrant storage (bb, page_idx returned in query)
            chunk_metadata = {"page_idx": page_idx}
            if bbox is not None:
                chunk_metadata["bb"] = bbox
                chunk_metadata["bbox"] = bbox

            # Build ZeRAG standard chunk format
            chunks[chunk_id] = {
                ...
                "file_path": file_ref,
                "llm_cache_list": [],  # ZeRAG will populate this field
    ...
    async def _store_chunks_to_zedata_type_aware(
        ...
        try:
            ...
            await self.zerag.text_chunks.upsert(chunks)

            # Store in chunks vector database for retrieval
            await self.zerag.chunks_vdb.upsert(chunks)
    ...
    async def _store_multimodal_main_entities(
        ...
        Args:
            ...
            zerag_chunks: Chunks in ZeRAG format (already formatted with templates)
        ...
        for data in multimodal_data_list:
            ...
            # Generate chunk_id using the formatted content (same as in _convert_to_zerag_chunks)
            chunk_id = compute_mdhash_id(formatted_chunk_content, prefix="chunk-")

            # Generate entity_id using ZeRAG's standard format
            entity_id = compute_mdhash_id(entity_name, prefix="ent-")

            # Create entity data in ZeRAG format
            entity_data = {
        ...
        if entities_to_store:
            try:
                ...
                for entity_id, entity_data in entities_to_store.items():
                    ...
                    await self.zerag.chunk_entity_relation_graph.upsert_node(
                        entity_name, node_data
                    )

                # Store in entities_vdb
                await self.zerag.entities_vdb.upsert(entities_to_store)
                await self.zerag.entities_vdb.index_done_callback()

                # NEW: Store multimodal main entities in full_entities storage
                if doc_id and self.zerag.full_entities:
    ...
    async def _store_multimodal_entities_to_full_entities(
        ...
        try:
            ...
            current_doc_entities = await self.zerag.full_entities.get_by_id(doc_id)
            ...
            # Store updated data
            await self.zerag.full_entities.upsert({doc_id: doc_entities_data})
            await self.zerag.full_entities.index_done_callback()
    ...
    async def _batch_extract_entities_zerag_style_type_aware(
        self, zerag_chunks: Dict[str, Any]
    ) -> List[Tuple]:
        """Use ZeRAG's extract_entities for batch entity relation extraction"""
        from app.core.kg.shared_storage import (
            get_namespace_data,
            get_pipeline_status_lock,
        )
        from app.core.operate import extract_entities

        # Get pipeline status (consistent with ZeRAG)
        pipeline_status = await get_namespace_data("pipeline_status")
        pipeline_status_lock = get_pipeline_status_lock()

        # Directly use ZeRAG's extract_entities
        chunk_results = await extract_entities(
            ...
            pipeline_status_lock=pipeline_status_lock,
            llm_response_cache=self.zerag.llm_response_cache,
            text_chunks_storage=self.zerag.text_chunks,
        )

        self.logger.info(
            f"Extracted entities from {len(zerag_chunks)} multimodal chunks"
        )
    ...
    async def _batch_add_belongs_to_relations_type_aware(
        ...
        for data in multimodal_data_list:
            ...
            # Use the same template formatting as in _convert_to_zerag_chunks_type_aware
            formatted_chunk_content = self._apply_chunk_template(
                content_type, original_item, description
            )
            chunk_id = compute_mdhash_id(formatted_chunk_content, prefix="chunk-")
    ...
    async def _batch_merge_zerag_style_type_aware(
        self, enhanced_chunk_results: List[Tuple], file_path: str, doc_id: str = None
    ):
        """Use ZeRAG's merge_nodes_and_edges for batch merge"""
        from app.core.kg.shared_storage import (
            get_namespace_data,
            get_pipeline_status_lock,
        )
    ...
    async def is_document_fully_processed(self, doc_id: str) -> bool:
        ...
        """
        try:
            doc_status = await self.zerag.doc_status.get_by_id(doc_id)
            if not doc_status:
                return False
    ...
    async def get_enriched_content(
        ...
        **kwargs,
        ...
        """
        Parse document and enrich multimodal content with descriptions,
        returning a single unified markdown string.
        ...
        self.logger.info(f"Generating enriched content for: {file_path}")

        # Strip doc_id from kwargs as it's not used by parse_document and causes issues in lower-level parsers
        kwargs.pop("doc_id", None)

        # Step 1: Parse document
        content_list, doc_id = await self.parse_document(
            file_path, output_dir, parse_method, display_stats, **kwargs
        )

        # Step 2: Set content source for context extraction
        if hasattr(self, "set_content_source_for_context"):
    ...
    async def process_document_complete(
        ...
        **kwargs,
        ...
        text_content, multimodal_items, text_items = separate_content(content_list)
        ...
        if kwargs.get("return_content", False):
            ...
            # However, to avoid double LLM calls in this specific path, we'd need to cache.
            # But normally process_document_complete is for legacy standalone runs.
            pass

    async def process_document_complete_zerag_api(
        self,
        file_path: str,
        output_dir: str = None,
        parse_method: str = None,
        ...
        **kwargs,
        ...
        """
        API exclusively for ZeRAG calls: Complete document processing workflow
        ...
        try:
            # Ensure ZeRAG is initialized
            result = await self._ensure_zerag_initialized()
            ...
            if pipeline_status_lock and pipeline_status:
                try:
                    async with pipeline_status_lock:
                        ...
                        error_msg = (
                            f"MultiRAG processing failed for {file_name}: {str(e)}"
                        )
            ...
            async with pipeline_status_lock:
                ...
                pipeline_status["latest_message"] = (
                    f"MultiRAG processing completed for {file_name}"
                )
                pipeline_status["history_messages"].append(
                    f"MultiRAG processing completed for {file_name}"
                )
