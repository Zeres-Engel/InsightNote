...
def separate_content(
    ...
    Returns:
        (text_content, multimodal_items, text_items): Pure text content, multimodal items list, and text items with metadata
    """
    text_parts = []
    multimodal_items = []
    text_items = []
    ...
    logger.info(f"  - Text items count (with metadata): {len(text_items)}")
    ...
    return text_content, multimodal_items, text_items
...
async def insert_text_content(
    ...
    """
    Insert text content into ZeRAG

    Args:
        zerag: ZeRAG instance
        input: Single document string, list of document strings, or list of dicts with 'text' and 'metadata'
    ...
    """
    logger.info("Starting text content insertion into ZeRAG...")

    # If input is a list of dictionaries (MinerU items with metadata)
    if isinstance(input, list) and len(input) > 0 and isinstance(input[0], dict):
        logger.info(f"Inserting {len(input)} custom blocks with metadata...")
        text_chunks = [item.get("text", "") for item in input]
        metadatas = [
            {"bbox": item.get("bbox"), "page_idx": item.get("page_idx")}
            for item in input
        ]
        full_text = "\n\n".join(text_chunks)
        doc_id = ids if isinstance(ids, str) else (ids[0] if ids else None)

        # We will update ainsert_custom_chunks in ZeRAG to handle metadata
        if hasattr(zerag, "ainsert_custom_chunks"):
            await zerag.ainsert_custom_chunks(
                full_text=full_text,
        ...
        # Use ZeRAG's standard insert method
        await zerag.ainsert(
            input=input,
            file_paths=file_paths,
            split_by_character=split_by_character,
...
async def insert_text_content_with_multimodal_content(
    ...
    """
    Insert pure text content into ZeRAG
    ...
    # Use ZeRAG's insert method with all parameters
    try:
        await zerag.ainsert(
            input=input,
        ...
        logger.info(
            "If the error is caused by the ainsert function not having a multimodal content parameter, please update the multirag branch of zerag"
        )
