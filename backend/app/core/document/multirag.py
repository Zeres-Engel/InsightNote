...
2. Pure text content ZeRAG insertion
...
# Load environment variables from .env file BEFORE importing ZeRAG
...
from app.core.document.query import QueryMixin
from app.core.document.processor import ProcessorMixin
from app.core.document.batch import BatchMixin
from app.core.document.utils import get_processor_supports
from app.core.document.parser import MineruParser, SUPPORTED_PARSERS, get_parser

# Import specialized processors
from app.core.document.modalprocessors import (
    ImageModalProcessor,
    TableModalProcessor,
...
@dataclass
class MultiRAG(QueryMixin, ProcessorMixin, BatchMixin):
    """Multimodal Document Processing Pipeline - Complete document parsing and insertion pipeline"""

    # Core Components
    # ---
    zerag: Optional[ZeRAG] = field(default=None)
    """Optional pre-initialized ZeRAG instance."""
    ...
    config: Optional[MultiRAGConfig] = field(default=None)
    """Configuration object, if None will create with environment variables."""

    # ZeRAG Configuration
    # ---
    zerag_kwargs: Dict[str, Any] = field(default_factory=dict)
    """Additional keyword arguments for ZeRAG initialization when zerag is not provided.
    This allows passing all ZeRAG configuration parameters like:
    ...
    """Parse result cache storage using ZeRAG KV storage."""
    ...
    def __post_init__(self):
        """Post-initialization setup following ZeRAG pattern"""
        # Initialize configuration if not provided
        if self.config is None:
            self.config = MultiRAGConfig()
        ...
        self.logger.info("MultiRAG initialized with config:")
    ...
    def close(self):
        ...
        try:
            ...
            print(f"Warning: Failed to finalize MultiRAG storages: {e}")

    def _create_context_config(self) -> ContextConfig:
        """Create context configuration from MultiRAG config"""
    ...
    def _create_context_extractor(self) -> ContextExtractor:
        """Create context extractor with tokenizer from ZeRAG"""
        if self.zerag is None:
            raise ValueError(
                "ZeRAG must be initialized before creating context extractor"
            )

        context_config = self._create_context_config()
        return ContextExtractor(
            config=context_config, tokenizer=self.zerag.tokenizer
        )
    ...
    def _initialize_processors(self):
        ...
        if self.zerag is None:
            raise ValueError(
                "ZeRAG instance must be initialized before creating processors"
            )
    ...
    async def _ensure_zerag_initialized(self):
        """Ensure ZeRAG instance is initialized, create if necessary"""
        try:
            ...
            if self.zerag is not None:
                # ZeRAG was pre-provided, but we need to ensure it's properly initialized
                # Inherit model functions from ZeRAG if not explicitly provided
                if self.llm_model_func is None and hasattr(
                    self.zerag, "llm_model_func"
                ):
                    self.llm_model_func = self.zerag.llm_model_func
                    self.logger.debug("Inherited llm_model_func from ZeRAG instance")

                if self.embedding_func is None and hasattr(
                    self.zerag, "embedding_func"
                ):
                    self.embedding_func = self.zerag.embedding_func
                    self.logger.debug("Inherited embedding_func from ZeRAG instance")

                try:
                    # Ensure ZeRAG storages are initialized
                    if (
                        not hasattr(self.zerag, "_storages_status")
                        or self.zerag._storages_status.name != "INITIALIZED"
                    ):
                        self.logger.info(
                            "Initializing storages for pre-provided ZeRAG instance"
                        )
                        await self.zerag.initialize_storages()
                    ...
                    if self.parse_cache is None:
                        self.logger.info(
                            "Initializing parse cache for pre-provided ZeRAG instance"
                        )
                        self.parse_cache = (
                            self.zerag.key_string_value_json_storage_cls(
                                namespace="parse_cache",
                                workspace=self.zerag.workspace,
                    ...
                    error_msg = (
                        f"Failed to initialize pre-provided ZeRAG instance: {str(e)}"
                    )
                    self.logger.error(error_msg, exc_info=True)
                    return {"success": False, "error": error_msg}

            # Validate required functions for creating new ZeRAG instance
            if self.llm_model_func is None:
                error_msg = "llm_model_func must be provided when ZeRAG is not pre-initialized"
                self.logger.error(error_msg)
                return {"success": False, "error": error_msg}

            if self.embedding_func is None:
                error_msg = "embedding_func must be provided when ZeRAG is not pre-initialized"
                self.logger.error(error_msg)
                return {"success": False, "error": error_msg}

            from app.core.kg.shared_storage import initialize_pipeline_status

            # Prepare ZeRAG initialization parameters
            zerag_params = {
            ...
            # Merge user-provided zerag_kwargs, which can override defaults
            zerag_params.update(self.zerag_kwargs)

            # Log the parameters being used for initialization (excluding sensitive data)
            log_params = {
                k: v
                for k, v in zerag_params.items()
                if not callable(v)
                and k not in ["llm_model_kwargs", "vector_db_storage_cls_kwargs"]
            }
            self.logger.info(f"Initializing ZeRAG with parameters: {log_params}")

            try:
                # Create ZeRAG instance with merged parameters
                self.zerag = ZeRAG(**zerag_params)
                await self.zerag.initialize_storages()
                await initialize_pipeline_status()

                # Initialize parse cache storage using ZeRAG's KV storage
                self.parse_cache = self.zerag.key_string_value_json_storage_cls(
                    namespace="parse_cache",
                    workspace=self.zerag.workspace,
                    global_config=self.zerag.__dict__,
                    embedding_func=self.embedding_func,
                ...
                # Initialize processors after ZeRAG is ready
                self._initialize_processors()

                self.logger.info(
                    "ZeRAG, parse cache, and multimodal processors initialized"
                )
                return {"success": True}

            except Exception as e:
                error_msg = f"Failed to initialize ZeRAG instance: {str(e)}"
                self.logger.error(error_msg, exc_info=True)
                return {"success": False, "error": error_msg}

        except Exception as e:
            error_msg = f"Unexpected error during ZeRAG initialization: {str(e)}"
            self.logger.error(error_msg, exc_info=True)
            return {"success": False, "error": error_msg}

    async def finalize_storages(self):
        """Finalize all storages including parse cache and ZeRAG storages

        This method should be called when shutting down to properly clean up resources
        and persist any cached data. It will finalize both the parse cache and ZeRAG's
        internal storages.

        Example usage:
            try:
                multi_rag = MultiRAG(...)
                await multi_rag.process_file("document.pdf")
                # ... other operations ...
            finally:
                # Always finalize storages to clean up resources
                if multi_rag:
                    await multi_rag.finalize_storages()
        ...
        try:
            ...
            # Finalize ZeRAG storages if ZeRAG is initialized
            if self.zerag is not None:
                tasks.append(self.zerag.finalize_storages())
                self.logger.debug("Scheduled ZeRAG storages finalization")

            # Run all finalization tasks concurrently
            if tasks:
                await asyncio.gather(*tasks)
                self.logger.info("Successfully finalized all MultiRAG storages")
            else:
    ...
    def get_config_info(self) -> Dict[str, Any]:
        ...
        # Add ZeRAG configuration if available
        if self.zerag_kwargs:
            # Filter out sensitive data and callable objects for display
            safe_kwargs = {
                k: v
                for k, v in self.zerag_kwargs.items()
                if not callable(v)
                and k not in ["llm_model_kwargs", "vector_db_storage_cls_kwargs"]
            }
            config_info["zerag_config"] = {
                "custom_parameters": safe_kwargs,
                "note": "ZeRAG will be initialized with these additional parameters",
            }
        else:
            config_info["zerag_config"] = {
                "custom_parameters": {},
                "note": "Using default ZeRAG parameters",
            }
    ...
    def update_context_config(self, **context_kwargs):
        ...
        if self.zerag and self.modal_processors:
            try:
                self.context_extractor = self._create_context_extractor()
