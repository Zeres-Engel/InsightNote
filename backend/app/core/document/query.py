...
class QueryMixin:
    """QueryMixin class containing query functionality for MultiRAG"""
    ...
    async def aquery(
        ...
        """
        Pure text query - directly calls ZeRAG's query functionality
        ...
        if self.zerag is None:
            raise ValueError(
                "No ZeRAG instance available. Please process documents first or provide a pre-initialized ZeRAG instance."
            )
        ...
        # Call ZeRAG's query method
        result = await self.zerag.aquery(
    ...
    async def aquery_with_multimodal(
        ...
        **kwargs,
        ...
        if (
            hasattr(self, "zerag")
            and self.zerag
            and hasattr(self.zerag, "llm_response_cache")
            and self.zerag.llm_response_cache
        ):
            if self.zerag.llm_response_cache.global_config.get(
                "enable_llm_cache", True
            ):
                try:
                    cached_result = await self.zerag.llm_response_cache.get_by_id(
                        cache_key
                    )
        ...
        if (
            ...
            try:
                await self.zerag.llm_response_cache.index_done_callback()
            except Exception as e:
    ...
    async def aquery_vlm_enhanced(
        ...
        **kwargs,
        ...
        Args:
            query: User query
            mode: Underlying ZeRAG query mode
            system_prompt: Optional system prompt to include
        ...
        if not hasattr(self, "vision_model_func") or not self.vision_model_func:
            raise ValueError(
                ...
                "Please provide a vision model function when initializing MultiRAG."
            )
        ...
        raw_prompt = await self.zerag.aquery(query, param=query_param)

        self.logger.debug("Retrieved raw prompt from ZeRAG")
        ...
        if not images_found:
            ...
            query_param = QueryParam(mode=mode, **kwargs)
            return await self.zerag.aquery(
                query, param=query_param, system_prompt=system_prompt
            )
